...
class Client:
    ...
    def move_player(self, napravlenie, znachenie):
        self.sock.sendall(bytes(json.dumps({'request': 'move',
                                            'napravlenie': napravlenie,
                                            'znachenie': znachenie}), 'UTF-8'))

    def shoot(self, pos):
        self.sock.sendall(bytes(json.dumps({'request': 'shoot',
                                            'pos': pos}), 'UTF-8'))
    ...
    def click(self, pos, funct):
        self.sock.sendall(bytes(json.dumps({'request': 'click',
                                            "pos": pos,
                                            'funct': funct}), 'UTF-8'))
